...
class CDGIN(tf.keras.Model):
    ...
    def __init__(self,config,return_hv=False):
        super(CDGIN, self).__init__(name="CDGIN")
        ...
        self.eps_edges = tf.Variable(tf.zeros((self.config.passing_hidden_size,)),trainable=True)
    ...
    def call(self,instance,train=True):
        try:
            ...
            for t in range(0,self.massge_iteration_dmpnn):
                ...
                h_t_vw = tf.nn.relu(tf.math.add(((1+self.eps_edges)*h_0_vw),self.passing[t](m_t_vw)))
